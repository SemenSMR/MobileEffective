...
import com.example.mobileeffective.exception.IllegalArgumentException;
    ...
    public Long extractUserId(String token) {
        try {
            ...
            if (userIdObject != null) {
                ...
                throw new IllegalArgumentException("Ключ 'userId' отсутствует или его значение равно null в токене JWT.");
            }
        } catch (MalformedJwtException e) {
            throw new InvalidInputException("Не удается извлечь идентификатор пользователя из токена JWT. Возможно, токен неправильно сформирован или срок его действия истек.");
        } catch (Exception e) {
            throw new IllegalArgumentException("Не удается извлечь идентификатор пользователя из токена JWT.");
        }
    }
